...
### Step 2: Gather data from websites

**If family_name (姓):**

1. Visit `https://japanese-names.info/last-name/{romaji_lowercase}/`
   - Extract: household count, English syllables, Japanese morae, kanji meanings (per-character breakdown), feature tags, famous persons list

2. Visit `https://myoji-yurai.net/searchResult.htm?myojiKanji={kanji_urlencoded}`
   - Extract: 【読み】(alternative readings), 【全国順位】(national rank), 【全国人数】(population), 由来解説 (etymology), 地域 TOP5 (regional distribution), 有名人 (famous people with name_jp + birth year + category)

**If given_name (名):**

1. Visit `https://japanese-names.info/first-name/{romaji_lowercase}/`
   - Extract: kanji meanings, feature tags, famous persons

2. myoji-yurai.net is mainly for surnames — skip for given names. Use LLM knowledge instead.

### Step 3: Combine data and fill all fields

Use data from websites + LLM's own knowledge to fill:

**From websites (hard data):**
- `mora_count` — from japanese-names.info "Japanese Morae" or count from hiragana reading
- `kanji_count` — count kanji characters in the `kanji` field
- `household_count` — from japanese-names.info "Households"
- `estimated_population` — from myoji-yurai.net 【全国人数】
- `national_rank` — from myoji-yurai.net 【全国順位】
- `alternative_readings` — from myoji-yurai.net 【読み】(exclude the primary reading already in `reading`)
- `kanji_breakdown` — from japanese-names.info kanji meanings, structured as JSON
- `famous_bearers` — combine both sites. `name_jp` MUST come from website data, never let LLM guess kanji for a person's name. Pick top 3-5 most notable people.
- `regional_origin` — from myoji-yurai.net 由来解説 + regional distribution

**From LLM knowledge (soft tags):**
- `era` — which era the name is most associated with
- `popularity` — can also be derived from estimated_population
...
- `etymology_en` — historical origin story

**Important rules:**
- `name_jp` in `famous_bearers` must come from website data — LLM should NOT guess kanji spellings for person names
...
## 4. Tag Enum Values (Complete Reference)
